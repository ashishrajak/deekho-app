...
//   final TextEditingController _offeringCodeController = TextEditingController();
...
//   final TextEditingController _estimatedCompletionController = TextEditingController();
//   Map<String, int> _mediaDisplayOrder = {}; // To track media display order
//   List<String> _existingMediaUrls = []; // To track existing media URLs
//   Map<String, String> _conditions = {}; // For pricing conditions
//   String? _advancePaymentMethod; // For advance payment method
...
//     _updateControllersFromDTO(widget.offering);
//   }
...
//       _status = dto.status ?? 'ACTIVE'; // Default to ACTIVE if null
//       _verified = dto.verified ?? false; // Default to false if null

//       // Offering Code (nullable)
//       _offeringCodeController.text = dto.offeringCode ?? '';
...
//         _emergencyService = timing.emergencyService ?? false;

//         // Estimated Completion (nullable DateTime)
//         // _estimatedCompletionController.text = timing.estimatedCompletion != null
//         //     ? DateFormat('yyyy-MM-dd').format(timing.estimatedCompletion!)
//         //     : '';

//         // Available Days - safely initialize
//         _availableDays = {
//           'Monday': timing.availableDays?.contains('Monday') ?? false,
//           'Tuesday': timing.availableDays?.contains('Tuesday') ?? false,
//           'Wednesday': timing.availableDays?.contains('Wednesday') ?? false,
//           'Thursday': timing.availableDays?.contains('Thursday') ?? false,
//           'Friday': timing.availableDays?.contains('Friday') ?? false,
//           'Saturday': timing.availableDays?.contains('Saturday') ?? false,
//           'Sunday': timing.availableDays?.contains('Sunday') ?? false,
//         };
//       } else {
//         // Initialize with defaults if timing is null
//         _durationController.clear();
//         _startTimeController.clear();
//         _endTimeController.clear();
//         _emergencyService = false;
//         _estimatedCompletionController.clear();
//         _availableDays = {
//           'Monday': false,
//           'Tuesday': false,
//           'Wednesday': false,
...
//         _onSiteService = geography.onSiteService ?? true; // Default to true
//         _willingToTravel = geography.willingToTravel ?? false;
//         _travelRadiusController.text = geography.travelRadius?.toString() ?? '';
//       } else {
//         // Initialize with defaults if geography is null
//         _latitude = null;
//         _longitude = null;
//         _serviceAreaController.clear();
//         _onSiteService = true;
//         _willingToTravel = false;
//         _travelRadiusController.clear();
//       }

//       // Tags - safely initialize empty set if null
//       _tags = dto.tags ?? <String>{};
...
//         // Base Price - required field
//         _basePriceController.text = pricing.basePrice.amount.toString();
//         _currencyController.text = pricing.basePrice.currency;

//         // Additional Charges (nullable)
//         _additionalChargesController.text = pricing.additionalCharges ?? '';

//         // Discount (nullable)
//         _discountController.text = pricing.discount ?? '';

//         // Payment Methods - safely initialize
//         _paymentMethods = {
//           'Cash': pricing.paymentMethods?.contains('Cash') ?? false,
//           'UPI': pricing.paymentMethods?.contains('UPI') ?? false,
//           'Credit/Debit Card': pricing.paymentMethods?.contains('Credit/Debit Card') ?? false,
//           'Online Payment': pricing.paymentMethods?.contains('Online Payment') ?? false,
//         };

//         // Conditions (nullable map)
//         _conditions = pricing.conditions ?? {};
//       } else {
//         // Initialize with defaults if pricing is null
//         _priceType = 'FIXED';
...
//         // Advance Payment Method (nullable)
//         _advancePaymentMethod = advancePolicy.advancePaymentMethod;

//         // Advance Terms (nullable)
//         _advanceTermsController.text = advancePolicy.advanceTerms ?? '';
//       } else {
//         _advanceRequired = false;
//         _advanceAmountController.clear();
//         _advanceCurrencyController.clear();
//         _advancePaymentMethod = null;
//         _advanceTermsController.clear();
//       }

//       // Media Items - safely initialize empty list if null
//       _existingMediaUrls = dto.mediaItems?.map((m) => m.mediaUrl).toList() ?? [];
//       _mediaDisplayOrder = dto.mediaItems?.fold<Map<String, int>>({}, (map, item) {
//         map[item.mediaUrl] = item.displayOrder;
//         return map;
//       }) ?? {};
//     });
//   }
